<?php

return [
    // Classes
    'classes' => 'Classes',
    'class' => 'Class',
    'add_class' => 'Add Class',
    'new_class' => 'New Class',
    'edit_class' => 'Edit Class',
    'class_details' => 'Class Details',
    'class_list' => 'Class List',
    'class_information' => 'Class Information',
    'class_name' => 'Class Name',
    'class_code' => 'Class Code',
    'class_fee' => 'Class Fee',
    'start_date' => 'Start Date',
    'end_date' => 'End Date',
    'max_students' => 'Max Students',
    'instructor_name' => 'Instructor Name',
    'instructor' => 'Instructor',
    'enrolled_students' => 'Enrolled Students',
    'total_classes' => 'Total Classes',
    'total_students' => 'Total Students',
    'total_paid' => 'Total Paid',
    'total_due' => 'Total Due',
    'required_amount' => 'Required Amount',
    'paid_amount' => 'Paid Amount',
    'due_amount' => 'Due Amount',
    'remaining' => 'Remaining',
    'collection_rate' => 'Collection Rate',
    'actions' => 'Actions',
    'view_details' => 'View Details',
    'no_classes' => 'No Classes Found',
    'add_new_class' => 'Add New Class',
    'add_new_class_to_start' => 'Add a new class to get started',
    'back_to_list' => 'Back to List',
    'save_class' => 'Save Class',
    'save_changes' => 'Save Changes',
    'delete_class' => 'Delete Class',
    'confirm_delete' => 'Confirm Delete',
    'class_created' => 'Class created successfully',
    'class_updated' => 'Class updated successfully',
    'class_deleted' => 'Class deleted successfully',

    // Class status
    'registration' => 'Registration',
    'class_status' => 'Class Status',
    'unlimited' => 'Unlimited',
    'not_specified' => 'Not specified',

    // Financial
    'enroll_student' => 'Enroll Student',
    'enroll_new_student' => 'Enroll New Student',
    'select_student' => 'Select Student',
    'choose_student' => 'Choose a student',
    'enrollment_date' => 'Enrollment Date',
    'total_amount' => 'Total Amount',
    'no_enrolled_students' => 'No Enrolled Students',
    'start_enrolling_students' => 'Start enrolling students in this class',
    'payment_history' => 'Payment History',
    'payment_method' => 'Payment Method',
    'no_payments_yet' => 'No payments yet',
    'add_payment' => 'Add Payment',
    'view_payments' => 'View Payments',
    'showing_payments' => 'Showing :current of :total payments',
    'export_enrollments' => 'Export Students',
    'export_filename' => ':class-enrollments-:date.xlsx',
    'course' => 'Course',
    'department' => 'Department',
    'instructor' => 'Instructor',
    'description' => 'Description',
    'date' => 'Date',
    'amount' => 'Amount',
    'notes' => 'Notes',
    'payment_status' => 'Payment Status',

    // Payment status
    'paid' => 'Paid',
    'partial' => 'Partial',
    'unpaid' => 'Unpaid',

    // Warnings
    'enrolled_students_warning' => 'Warning: There are enrolled students. Changing fees may affect payments',
    'currently_enrolled' => 'Currently enrolled: :count',
    'cannot_undo' => 'This action cannot be undone',
    'confirm_delete_class' => 'Are you sure you want to delete this class?',
    'edit_class_information' => 'Edit Class Information',
    
    // Validation messages
    'class_name_required' => 'Class name is required.',
    'class_name_max' => 'Class name may not be greater than 255 characters.',
    'class_code_required' => 'Class code is required.',
    'class_code_max' => 'Class code may not be greater than 50 characters.',
    'class_code_unique' => 'This class code is already taken.',
    'course_required' => 'Please select a course.',
    'course_invalid' => 'Please select a valid course.',
    'category_required' => 'Please select a category.',
    'category_invalid' => 'Please select a valid category.',
    'start_date_required' => 'Start date is required.',
    'start_date_invalid' => 'Please enter a valid start date.',
    'end_date_required' => 'End date is required.',
    'end_date_invalid' => 'Please enter a valid end date.',
    'end_date_after_start' => 'End date must be after start date.',
    'status_required' => 'Please select a status.',
    'status_invalid' => 'Please select a valid status.',
    'class_fee_numeric' => 'Class fee must be a number.',
    'class_fee_min' => 'Class fee must be at least 0.',
    'max_students_integer' => 'Max students must be a whole number.',
    'max_students_min' => 'Max students must be at least 1.',
    'instructor_name_max' => 'Instructor name may not be greater than 255 characters.',
    
    // Cascading dropdowns
    'first_select_department' => 'First select a department',
    'no_courses_in_department' => 'No courses available in this department',
    
    // Moodle LMS Integration
    'moodle_course' => 'Moodle Course',
    'select_moodle_course' => 'Select Moodle Course',
    'moodle_course_help' => 'Optional: Link this class to a Moodle LMS course',
    'current_moodle_course' => 'Current Moodle Course',
    'no_moodle_courses' => 'No Moodle courses available',
    'error_loading_moodle' => 'Error loading Moodle courses',
    'moodle_status' => 'Moodle Status',
    'sync_to_moodle' => 'Sync to Moodle',
    'moodle_status_synced' => 'Synced',
    'moodle_status_syncing' => 'Syncing…',
    'moodle_status_failed' => 'Failed',
    'moodle_status_not_synced' => 'Not synced',
    'moodle_sync_created_user' => 'Student created in Moodle and enrolled successfully.',
    'moodle_sync_existing_user' => 'Existing Moodle user enrolled successfully.',
    'moodle_sync_failed' => 'Unable to sync with Moodle.',
    'moodle_sync_missing_course' => 'This class is not linked to a Moodle course.',
    'moodle_sync_missing_student' => 'Student record is missing or inactive.',
    'moodle_sync_inactive_enrollment' => 'Only active enrollments can be synced to Moodle.',
    'moodle_sync_missing_email' => 'An email address is required to create a Moodle account.',
];